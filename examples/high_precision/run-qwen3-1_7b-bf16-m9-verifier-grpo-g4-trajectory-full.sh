...
# M9-P. Full fine-tune. Orbit re-implementation of the TRL/accelerate
...
# === Recipe identity ===
LAUNCHER_NAME=run_qwen3_17b_m9_verifier_grpo_g4_trajectory_full
...
# === Training schedule ===
# 64 unique prompts x G=4 = 256 trajectories, all consumed in one optimizer
# step per rollout round (matches TRL's num_iterations: 1 / generation_batch_size: 256).
NUM_ROLLOUT="${NUM_ROLLOUT:-300}"          # == max_steps: 300
ROLLOUT_BATCH_SIZE="${ROLLOUT_BATCH_SIZE:-64}"    # unique prompts / rollout
N_SAMPLES_PER_PROMPT="${N_SAMPLES_PER_PROMPT:-4}" # G
GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-256}"     # trajectories / optimizer step
