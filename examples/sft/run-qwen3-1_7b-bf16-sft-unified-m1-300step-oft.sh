#!/usr/bin/env bash
# Qwen3-1.7B OFT SFT, hyperparameters translated from the M1 300-step "hard
...
#
# NOTE: the source yaml has no PEFT/OFT section at all -- it's a full
# fine-tune. PEFT_ARGS below (canonical_oft, block size 128, eps 6e-5,
# all-linear) is a requested addition on top of the translated recipe, not
# something translated from the source; see
...
# === Recipe identity ===
LAUNCHER_NAME=${LAUNCHER_NAME:-unified_m1_sft_hard_non_thinking_300step_oft}
...
SAVE_DIR="${SAVE_DIR:-${ORBIT_ROOT}/orbit_ckpts/unified-m1-sft-hard-non-thinking-300step-oft}"
...
# OFT (source yaml is a full finetune; this PEFT config was requested
