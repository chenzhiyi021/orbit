#!/usr/bin/env bash
# Qwen3-1.7B full-parameter SFT, hyperparameters translated from a TRL
# SFTConfig-style YAML (accelerate + deepspeed_zero3, not this codebase):
#
#   accelerate_config: examples/accelerate_configs/deepspeed_zero3.yaml
#   model_name_or_path: Qwen/Qwen3-1.7B
#   dataset_name: ../data/unified_m0_m5/banks/teacher_non_thinking/dataset
#   output_dir: ../outputs/unified-m1-sft-hard-non-thinking
#   run_name: unified-m1-sft-hard-non-thinking
...
#   warmup_steps: 10 / max_grad_norm: 1.0 / weight_decay: 0.0
#   max_length: 12288
...
#   - max_steps 100                 -> NUM_ROLLOUT=100 (exact).
#   - learning_rate / cosine_with_min_lr / min_lr_rate 0.1 / warmup_steps 10
#     with max_steps=100            -> --lr-decay-style cosine --min-lr 5e-7
#                                       --lr-warmup-fraction 0.1 (10/100).
#                                       If you change NUM_ROLLOUT, recompute
#                                       the warmup fraction (or switch to
#                                       Megatron's --lr-warmup-iters 10 for an
#                                       absolute count that doesn't move).
...
#   - shuffle_dataset: false        -> --rollout-shuffle is NOT set below.
#   - per_device_train_batch_size 1 / gradient_accumulation_steps 32
...
#   - save_total_limit: 1           -> no equivalent flag found in this
...
# === Recipe identity ===
LAUNCHER_NAME=${LAUNCHER_NAME:-unified_m1_sft_hard_non_thinking}
...
SAVE_DIR="${SAVE_DIR:-${ORBIT_ROOT}/orbit_ckpts/unified-m1-sft-hard-non-thinking}"
# The source dataset_name (.../teacher_non_thinking/dataset) looks like a HF
# `datasets.save_to_disk()` directory, not a chat-format JSONL. Orbit's
# --prompt-data expects .jsonl/.parquet with a `messages` column (see
# examples/sft/README.md). Point TRAIN_JSONL at a converted file, or export
# the source dataset to jsonl first if it isn't already in that schema.
TRAIN_JSONL="${TRAIN_JSONL:?set TRAIN_JSONL to a chat-format (messages) jsonl/parquet path}"
...
# Full fine-tuning (source yaml has no PEFT/LoRA config).
PEFT_ARGS=(
    --peft-method none
)
