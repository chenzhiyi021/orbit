#!/usr/bin/env bash
# Qwen3-1.7B full-parameter SFT, hyperparameters translated from the M1
# 100-step "hard SFT" TRL SFTConfig-style YAML (accelerate + deepspeed_zero3,
# not this codebase). Two source variants were given and are IDENTICAL on
# every training hyperparameter below -- they only differ in which teacher
# rollout bank populates `dataset_name` / `output_dir` / `run_name`:
#
...
# 100-step cosine_with_min_lr recipe -- a DIFFERENT source yaml, not this one
# with fewer steps), this launcher does NOT read from a prebuilt HF
# `datasets.save_to_disk()` bank. It reads TRAIN_JSONL straight from
# tools/generate_teacher_sft_data.py's own --output: that tool already writes
# {"messages": [...], "metadata": {...}} rows, one per line, which is exactly
# Orbit's --input-key messages schema -- no bank/HF-dataset conversion step in
# between.
#
...
#                                       effective batch 32*8=256, hence 25,600
#                                       examples over 100 steps), set
...
#   - 25,600 total examples vs GLOBAL_BATCH_SIZE=32 * NUM_ROLLOUT=100 = 3,200:
...
#     process 25,600 examples over 100 steps, raise ROLLOUT_BATCH_SIZE and
...
# === Recipe identity ===
LAUNCHER_NAME=${LAUNCHER_NAME:-unified_m1_sft_hard_non_thinking_100step}
...
SAVE_DIR="${SAVE_DIR:-${ORBIT_ROOT}/orbit_ckpts/unified-m1-sft-hard-non-thinking-100step}"
